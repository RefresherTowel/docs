...
### 6. Custom buses (advanced)

You can create additional `PulseController` instances when you need isolated buses (for example, separate UI vs gameplay events, or per-scene buses in a tool). Most projects can stick to the global `PULSE`.

```js
// Create a custom bus (e.g., in a controller object or struct init)
ui_bus = new PulseController();

// Subscribe/send on the custom bus
ui_bus.Subscribe(id, SIG_UI_CLICK, OnUIClick);
ui_bus.Send(SIG_UI_CLICK, { x: 100, y: 50 }, id);

// Or use the builder, targeting the bus
var _cfg = PulseListener(id, SIG_UI_OPEN, OnUiOpen)
    .Bus(ui_bus)
    .Priority(5)
    .Tag("ui")
    .Subscribe();
...
- Each bus has its own listeners, counts, and queue; `PulsePost`/`PulseFlushQueue` equivalents exist as `bus.Post()` / `bus.FlushQueue()`.
- Keep bus references somewhere you can access them (for example, a manager object, a singleton, or a struct field).
- Use this only when you need strict separation; otherwise the global `PULSE` is simpler.
